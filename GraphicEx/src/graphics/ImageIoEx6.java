...
public class ImageIoEx6 extends JPanel {
	int width, height;
	Image[] images = new Image[20]; // 이미지를 저장할 배열
	int[] game = new int[20]; // 섞인 숫자가 저장될 배열
	Image srcImage;

	public ImageIoEx6() {
		try {
			srcImage = ImageIO.read(new File("stop.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		// 카드 1장의 이미지크기
		width = srcImage.getWidth(null) / 20;
	...
	public static void main(String[] args) {
		...
		ImageIoEx6 panel = new ImageIoEx6();
		frame.add(panel);
