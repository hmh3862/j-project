...
public class ImageIoEx5 extends JPanel {
	int width, height;
	Image[] images = new Image[52]; // 이미지를 저장할 배열
	int[] game = new int[52]; // 섞인 숫자가 저장될 배열
	Image srcImage;

	public ImageIoEx5() {
		try {
			srcImage = ImageIO.read(new File("gostop.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		// 카드 1장의 이미지크기
		width = srcImage.getWidth(null) / 52;
	...
	public static void main(String[] args) {
		...
		ImageIoEx5 panel = new ImageIoEx5();
		frame.add(panel);
